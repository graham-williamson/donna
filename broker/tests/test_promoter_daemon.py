...
def test_handle_install_ok() -> None:
    ...
    reply = d.handle_frame(
        _frame({"op": "install_pack", "pack_dir": "/p", "request_id": "R1"}),
...
def test_handle_missing_pack_dir_errors() -> None:
    def do_install(*, pack_dir: str, request_id: str) -> dict[str, str]:
        raise AssertionError("do_install must NOT run when pack_dir is missing")

    reply = d.handle_frame(
        _frame({"op": "install_pack", "request_id": "R"}), do_install=do_install
    )
    body = _decode(reply)
    assert body["ok"] is False
    assert "pack_dir" in body["error"]


def test_handle_missing_request_id_errors() -> None:
    def do_install(*, pack_dir: str, request_id: str) -> dict[str, str]:
        raise AssertionError("do_install must NOT run when request_id is missing")

    reply = d.handle_frame(
        _frame({"op": "install_pack", "pack_dir": "/p"}), do_install=do_install
    )
    body = _decode(reply)
    assert body["ok"] is False
...
def test_handle_non_string_fields_errors() -> None:
    def do_install(*, pack_dir: str, request_id: str) -> dict[str, str]:
        raise AssertionError("do_install must NOT run for wrong field types")

    reply = d.handle_frame(
        _frame({"op": "install_pack", "pack_dir": 5, "request_id": "R"}),
        do_install=do_install,
    )
    assert _decode(reply)["ok"] is False
...
def test_handle_install_raises_becomes_error_reply() -> None:
    def boom(*, pack_dir: str, request_id: str) -> dict[str, str]:
        raise RuntimeError("refused: bad sig")

    reply = d.handle_frame(
        _frame({"op": "install_pack", "pack_dir": "/p", "request_id": "R"}),
        do_install=boom,
    )
    body = _decode(reply)
    assert body["ok"] is False
...
def test_handle_promoter_error_reason_relayed() -> None:
    ...
    def refuse(*, pack_dir: str, request_id: str) -> dict[str, str]:
