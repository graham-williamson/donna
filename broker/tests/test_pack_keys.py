...
def _make_key(d, key_id, *, revoked=False):
...
def test_verify_accepts_signature_from_trusted_key(tmp_path):
...
def test_verify_rejects_signature_from_unknown_key(tmp_path):
...
def test_verify_rejects_revoked_key(tmp_path):
...
def test_load_trusted_keys_empty_dir_is_no_keys(tmp_path):
    store = pack_keys.load_trusted_keys(str(tmp_path))
    with pytest.raises(pack_keys.SignatureError):
        pack_keys.verify(store, b"m", b"\x00" * 64)


def test_load_trusted_keys_rejects_malformed_key_file(tmp_path):
...
def test_verify_rejects_wrong_length_signature(tmp_path):
...
def test_verify_returns_second_key_when_only_it_matches(tmp_path):
...
def test_load_trusted_keys_missing_dir_is_empty_store(tmp_path):
...
def test_load_trusted_keys_skips_revoked_but_keeps_others(tmp_path):
...
def test_load_trusted_keys_rejects_valid_hex_wrong_length(tmp_path):
...
def test_load_trusted_keys_rejects_unreadable_revoked_file(tmp_path):
...
def test_verify_rejects_revoked_key_even_though_pub_file_present(tmp_path):
